...
2. **Perform the accepted one-time forwarder bootstrap rerun.** The gate's forwarder list now names the demo workflow, and a forwarder cannot retry a workflow it did not know about on the run that introduced it. ~~PASS = the gate is re-run once on the merge head AFTER the forwarder change is present, and the second run forwards the demo retry.~~ **CORRECTED (Codex, Task 9 round 1): the struck condition describes an IMPOSSIBLE event** — the new `workflow_run` listener runs from the default branch and is unavailable until this PR merges, so a manual gate rerun does not use the new forwarder; `closeout-gate.yml:73` says exactly this. PASS = **after the demo completes, manually re-run the existing closeout-gate run on the exact PR head, and confirm the rerun no longer blocks on the demo.**
...
6. **Hosted demo result.** PASS = `.github/workflows/debug-evidence-demo.yml` completed on the hosted runner and the `strict-refusal` probe reached the ADMISSION REFUSAL (exit 3 from `start`) rather than an input error. This is the one place the strict default is exercised on the platform it was written for: hosted runners ship a passwordless-sudo rule, so `strict` MUST refuse there. **A `start` INVOCATION returning 0 under strict means the admission check is broken; the JOB itself is EXPECTED to pass, because its shell asserts the exit 3 — a green job confirms the refusal; a red job is NOT PASS and requires inspecting whether strict returned a non-3 status or the probe failed before confirming it** (a red job can equally be a checkout or setup failure, an input error exiting 1, or the assertion itself erroring — it means the refusal was not CONFIRMED, not that it did not occur). *(Reworded at Codex rounds 3 and 4: the first version — "a demo job that went green under strict means the admission check is broken" — condemned the job's expected outcome; the second — "a red job is the probe reporting that the refusal did not happen" — over-read a red job in the other direction.)*

   **SCOPE CORRECTION (Codex, Task 9 round 1): this item proves the ADMISSION REFUSAL and nothing more.** The `strict-refusal` job calls `support.js start` **directly** (`debug-evidence-demo.yml:258`) — it never invokes the composite action and has no wrapped-command step, so "the wrapped command never executed" was vacuous here, and this item **cannot** verify failure persistence, `always()` guards, or the `skipped` outcome value. The earlier claim that it settles the composite harness's semantic model is **retracted** — see the harness-limit paragraph below for what that model now rests on. A live composite-refusal probe is not buildable in this repo's gate design: a genuinely failed composite step cannot coexist with a green demo workflow, `continue-on-error` is forbidden, and Codex closed that search in Task 7.
7. **THE DOWNLOADED ARTIFACT — the last hop of check 2, and the only one that closes it.** Download the `debug-evidence` artifact from the demo run and check, in this order:
   ...
   d. ~~the artifact contains neither the session token nor the launch token — grep it for both, taking the session token from nowhere~~ **CORRECTED (Codex, Task 9 round 1): as written this was UNEXECUTABLE** — it demanded a grep for a value it simultaneously admitted cannot be obtained, while the preamble requires every item to PASS. The executable condition: **grep the downloaded artifact for `action-state.json` content signatures and for the credential KEY NAMES (`launchToken`, `sessionToken`, `launch_token`, `session_token`) — PASS = none present.** Raw-VALUE absence is established **locally** by check 3 (the values never reach the staged bytes) and is not re-proved from the download.
...
**AND THE HARNESS'S OWN LIMIT, stated because check 1 rests on it.** The three composite rules are the IMPLEMENTER'S READING of GitHub's precedence, verified against no hosted runner. The guard STRINGS are pinned by an existing test and the step list is parsed from the shipped file, so a rewiring cannot escape; what is unverified is the SEMANTICS — most sharply, that a step which never ran presents `steps.<id>.outcome` as `skipped` rather than as the empty string. **ATTRIBUTION CORRECTED (Codex, round 3): the case that turns on that reading is the MODEL ONLY skipped-start drive, and ONLY it.** Check 1's refusal case does NOT — its `start` RAN and carries outcome `failure`, so its guards evaluate on `failure != 'skipped'`, which is true under either reading of a never-run step. If the skipped reading is wrong, the MODEL ONLY drive models a precedence GitHub does not implement and every check-1 verdict is unaffected. ~~Merge-time item 6 is what settles it: the hosted `strict-refusal` job exercises exactly this path~~ **CORRECTED (Codex, Task 9 round 1): item 6 settles NOTHING about these semantics — the `strict-refusal` job invokes `support.js` directly and never runs the composite, and no live probe of a failed composite step is buildable under this repo's gate design.** What the model rests on, stated exactly: GitHub's published documentation lists `skipped` among the `steps.<id>.outcome` values and specifies `always()` ([expressions](https://docs.github.com/en/actions/reference/workflows-and-actions/expressions), [steps context](https://docs.github.com/en/enterprise-cloud%40latest/actions/reference/workflows-and-actions/contexts)) — so the reading is DOCUMENTATION-SUPPORTED and remains UNVERIFIED BY LIVE EXECUTION in this repo. It ships as a stated limitation. The one part that IS exercisable locally — the model's own false branch of `outcomes.start !== 'skipped'`, which none of the original six drives reached — is exercised by the skipped-start drive shipped at `cada897`, which proves the MODEL's consistency, not GitHub's behaviour.
...
#### Task 9 fix round 1 (Codex review of `6af6352`: 3 Important, 2 Minor) — the code held; the MERGE-TIME CONTRACT did not

**All three Important findings are against the checklist and the claims made about it — a document — and every one is a variant of the same defect: a PASS condition that cannot be executed as written.** The cycle spent fifteen rounds removing "it looked fine" from tests, then wrote three unexecutable conditions into the one artifact a human runs by hand.

**(1) Important — merge-time item 6 does not test what it claimed to settle.** The `strict-refusal` job calls `support.js start` DIRECTLY (`debug-evidence-demo.yml:258`); it never invokes the composite action and has no wrapped-command step, so "the wrapped command never executed" was vacuous and the item cannot verify failure persistence, `always()` guards, or the `skipped` outcome. **The plan's claim that this hosted job exercises "exactly this path" was false.** Also: no round-1 drive exercises the FALSE branch of `outcomes.start !== 'skipped'`. RESOLUTION (the second of Codex's two options, because the first is unbuildable here): the settling claim is RETRACTED at both sites; the semantic model is declared DOCUMENTATION-SUPPORTED (GitHub's expressions and steps-context pages list `skipped` and specify `always()`) and UNVERIFIED BY LIVE EXECUTION, shipping as a stated limitation; and a skipped-start drive is added to exercise the guard's false branch IN THE MODEL — proving the model's consistency, explicitly not GitHub's behaviour.

**(2) Important — the bootstrap PASS condition described an impossible event.** "The second run forwards the demo retry" — but the new `workflow_run` listener is unavailable until this PR merges (`closeout-gate.yml:73` explains this itself), so a manual gate rerun does not use the new forwarder. Corrected: after the demo completes, manually re-run the EXISTING closeout-gate run on the exact PR head; PASS = the rerun no longer blocks on the demo.

**(3) Important — artifact check 7d was unexecutable as written.** It required grepping for a session-token value it simultaneously admitted cannot be obtained, under a preamble requiring every item to PASS. Corrected to the executable condition: grep the download for `action-state.json` content signatures and credential KEY NAMES; raw-VALUE absence is established locally by check 3 and not re-proved from the download.

**(4) Minor — unknown `uses:` steps silently succeed in the driver** (`support.test.js:10859`). Unknown guards and expressions throw; a non-`run`, non-upload step becomes a successful no-op. The driver must admit ONLY the pinned setup-node step and the uploader, and throw on everything else — the same fail-closed posture the other two seams already have.

**(5) Minor — `driveCompositeAction` leaks its collector when the drive itself throws** (`stop()` is only reachable on the returned object; 27 orphans accumulated during round-1 mutation work and were reaped by hand). Cleanup moves inside the driver so a mid-drive throw reaps its own collector.
...
#### Task 9 fix round 3 (Codex review of `cada897`+`9fa4b7b`: 1 Important, 1 Minor) — the retraction itself needed retracting, in two half-sentences

**(1) Important — MY retraction text was internally contradictory at both remaining sites, and both defects are half-sentence attribution errors.** (a) Item 6 said "a demo job that went green under `strict` means the admission check is broken" — but the job goes green PRECISELY BY observing the refusal (its shell asserts exit 3), so the sentence condemned the expected outcome. Corrected to Codex's formulation: a `start` INVOCATION returning 0 under strict means breakage; the JOB is expected to pass. (b) The harness-limit paragraph attributed the skipped-semantics dependency to check 1's refusal case — whose `start` RAN with outcome `failure`, making its guards true under either reading of a never-run step. **The sole consumer of the skipped reading is the MODEL ONLY drive**, and "every other case is unaffected" was outdated the moment that drive shipped. Both corrected in place. The lesson, recorded because it is the same shape three times now: **each of rounds 1–3's Important findings was against prose I wrote ABOUT the verification, not against the verification** — the checklist, then the retraction, then the retraction's own attribution.

**(2) Minor — the outcome-map assertion proves the inputs exist, not which guard consumed which input.** In the skipped-start state the two facts are correlated (start skipped AND `evidence-dir` empty), so both predicates return false REGARDLESS of which body is on which guard — **swapping the two evaluator bodies passes every drive.** Fix: two orthogonal direct probes of `evaluateStepGuard` — (skipped start + POPULATED evidence-dir) and (failed start + EMPTY evidence-dir) — states where the two predicates ANSWER DIFFERENTLY, so a swapped implementation dies. The drive's comment claiming the outcome map "keeps those reasons distinct" is corrected to claim only what it proves.
...
#### Task 9 fix round 4 (Codex review of `6d4597e`+`b71349f`: 1 Minor) — the red-job clause over-read in the OTHER direction

**Codex confirmed everything else closes:** the harness-limit attribution is accurate; **the four probes complete the Boolean truth table — reachable equal states covered by drives, both unequal states covered directly, and "there is no third correlated state for these two predicates"**; the probes use the actual guard strings so the swap now fails; **"no code issue from rounds 1–3 remains open"**; merge base, scope and `diff --check` re-confirmed.
